...
6. **设置面板下方滑不到**：设置项较多，视觉自定义/执行模式等靠下配置被挤出窗口。已给设置内容区加独立滚动（`max-height: 46vh` + `overflow-y: auto`），实测 `canScroll: true`。

> 📘 **完整项目说明书见 [项目说明书.md](项目说明书.md)** —— 包含项目定位、功能详解、架构原理、逐项配置说明、完整实测数据、二次开发指南。
